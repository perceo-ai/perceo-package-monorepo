...
export async function callObserverBootstrapApi(
	...
	try {
		...
		if (!response.ok) {
			...
			throw new Error(`Bootstrap API call failed: ${response.status} ${response.statusText}. ${errorBody}`);
		}
...
export async function callObserverAnalyzeApi(
	...
	try {
		...
		if (!response.ok) {
			const errorBody = await response.text();
			throw new Error(`Analyze API call failed: ${response.status} ${response.statusText}. ${errorBody}`);
		}
...
export async function analyzeFileChange(
	...
	try {
		...
		if (!response.ok) {
			const errorBody = await response.text();
			throw new Error(`File analysis API call failed: ${response.status} ${response.statusText}. ${errorBody}`);
		}
...
export async function triggerAffectedTests(
	...
	try {
		...
		if (!response.ok) {
			const errorBody = await response.text();
			throw new Error(`Trigger tests API call failed: ${response.status} ${response.statusText}. ${errorBody}`);
		}
