...
export class TemporalClient {
  ...
  private async createClient(): Promise<Client> {
    ...
    const connection = await Connection.connect({
      ...
      tls: this.config.tls
        ? {
            clientCertPair: {
              crt: readFileSync(this.config.tls.certPath),
  ...
  async executeWorkflow<T>(
    ...
    // Start workflow
    const handle: WorkflowHandle<T> = await client.workflow.start(workflowName, {
      args: [input],
      taskQueue,
   ...
   */
  async getWorkflowHandle<T>(workflowId: string): Promise<WorkflowHandle<T>> {
    const client = await this.getClient();
    return client.workflow.getHandle<T>(workflowId);
  }
