...
export class PerceoDataClient {
	 ...
	 */
	static async fromUserSession(params: { supabaseUrl: string; supabaseKey: string; accessToken: string; refreshToken: string; projectId?: string }): Promise<PerceoDataClient> {
	 ...
	 */
	async upsertProjectSecret(projectId: UUID, keyName: string, value: string, createdBy?: UUID): Promise<UUID> {
	 ...
	 */
	async getProjectSecret(projectId: UUID, keyName: string): Promise<string | null> {
		...
		if (error) {
			// If the error is "insufficient permissions", return null
	...
	async updateProject(projectId: UUID, updates: Partial<ProjectInsert>): Promise<Project> {
		const { data, error } = await this.supabase
			.from("projects")
	 ...
	 */
	async getProjectMemberRole(projectId: UUID): Promise<ProjectRole | null> {
		const {
			data: { user },
	 ...
	 */
	async addProjectMember(projectId: UUID, userId: UUID, role: ProjectRole): Promise<ProjectMember> {
		const { data, error } = await this.supabase
	...
	async getPersonasBySource(source: "user_configured" | "auto_generated", projectId?: string): Promise<Persona[]> {
		const pid = projectId ?? this.projectId;
		if (!pid) throw new Error("Project ID required");
	...
	async createUserConfiguredPersonas(personas: PersonaInsert[], projectId?: string): Promise<Persona[]> {
		const pid = projectId ?? this.projectId;
		if (!pid) throw new Error("Project ID required");

		// Mark all personas as user_configured
		...
		const { data, error } = await this.supabase
			.from("personas")
			.insert(personasWithSource as any)
			.select();
	...
	async deleteAutoGeneratedPersonas(projectId?: string): Promise<void> {
		const pid = projectId ?? this.projectId;
		if (!pid) throw new Error("Project ID required");
	 ...
	 */
	async publishFlowsAffected(changeId: string, affectedFlows: { id: UUID; name: string; riskScore: number }[], source: EventSource = "observer"): Promise<PerceoEvent> {
	 ...
	 */
	async publishTestStatus(testRunId: UUID, flowId: UUID, status: string, source: EventSource = "coordinator"): Promise<PerceoEvent> {
		return this.publishEvent({
			type: `test.${status}`,
			payload: {
				testRunId,
